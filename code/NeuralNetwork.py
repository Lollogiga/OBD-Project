...
def train_model(X, Y, parameters, lambd, num_epochs, learning_rate, regularization="L2"):
    for epoch in range(num_epochs):
        # Forward pass
        AL, caches = L_layer_forward(X, parameters, "relu")  # Usa "relu" per gli strati intermedi

        # Calcola il costo
        cost = compute_cost(AL, Y, parameters, lambd, regularization)

        # Backward pass
        grads = L_layer_backward(AL, Y, caches, parameters, lambd, regularization)
        ...
        # Stampa il costo ogni 100 epoche
        if epoch % 100 == 0:
